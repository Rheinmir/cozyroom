...
func scanLibrary(h *handlers, w http.ResponseWriter) {
	// Music scan
	res, err := library.Scan(h.scanDB, h.musicPath, h.coversDir)
	if err != nil {
	...
	// Video scan
	if h.filmsPath != "" {
		_ = library.ScanVideos(h.scanDB, h.filmsPath)
	}

	// Ebook scan
	if h.ebooksPath != "" {
		_ = library.ScanEbooks(h.scanDB, h.ebooksPath, h.ebookCoversDir)
	}
