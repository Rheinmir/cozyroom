...
func (r *ComicsCacheRepo) SaveSearch(source, query string, results []byte) error {
	_, err := r.DB.Exec(`
		INSERT INTO comics_cache (source, query, results, updated_at)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT(source, query) DO UPDATE SET results=EXCLUDED.results, updated_at=EXCLUDED.updated_at
...
func (r *ComicsCacheRepo) GetSearch(source, query string, maxAge time.Duration) ([]byte, error) {
	...
	var updatedAt int64
	err := r.DB.QueryRow(`
		SELECT results, updated_at FROM comics_cache WHERE source = $1 AND query = $2
...
func (r *ComicsCacheRepo) SaveGallery(gid, token string, detail []byte) error {
	_, err := r.DB.Exec(`
		INSERT INTO comics_galleries (gid, token, detail, updated_at)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT(gid, token) DO UPDATE SET detail=EXCLUDED.detail, updated_at=EXCLUDED.updated_at
...
func (r *ComicsCacheRepo) GetGallery(gid, token string, maxAge time.Duration) ([]byte, error) {
	...
	var updatedAt int64
	err := r.DB.QueryRow(`
		SELECT detail, updated_at FROM comics_galleries WHERE gid = $1 AND token = $2
...
func (r *ComicsCacheRepo) SaveChapterPages(gid, chapID string, pages []byte) error {
	_, err := r.DB.Exec(`
		INSERT INTO comics_pages (manga_id, chapter_id, pages, updated_at)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT(chapter_id) DO UPDATE SET pages=EXCLUDED.pages, updated_at=EXCLUDED.updated_at
...
func (r *ComicsCacheRepo) GetChapterPages(chapID string, maxAge time.Duration) ([]byte, error) {
	...
	var updatedAt int64
	err := r.DB.QueryRow(`
		SELECT pages, updated_at FROM comics_pages WHERE chapter_id = $1
