...
func (r *LyricsCacheRepo) Get(ctx context.Context, trackID string) (string, error) {
	...
	err := r.DB.QueryRowContext(ctx,
		`SELECT results FROM lyrics_cache WHERE track_id = $1`, trackID).Scan(&jsonData)
	...
	_, err := r.DB.ExecContext(ctx,
		`INSERT INTO lyrics_cache (track_id, results, fetched_at)
		 ...
		 ON CONFLICT(track_id) DO UPDATE SET
		 	...
		 	fetched_at = EXTRACT(EPOCH FROM NOW())::INTEGER`,
		trackID, jsonData)
	return err
}
...
func (r *LyricsCacheRepo) Delete(ctx context.Context, trackID string) error {
	_, err := r.DB.ExecContext(ctx,
		`DELETE FROM lyrics_cache WHERE track_id = $1`, trackID)
	return err
}
