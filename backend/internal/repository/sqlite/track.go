...
// NewTrackRepo creates a repo backed by the given DB (read path).
...
func (r *TrackRepo) GetFilePath(ctx context.Context, id string) (string, error) {
	var fp string
	err := r.q.QueryRowContext(ctx, "SELECT file_path FROM tracks WHERE id = ?", id).Scan(&fp)
	if err == sql.ErrNoRows {
		return "", nil
	}
	return fp, err
...
func (r *TrackRepo) SmartQueue(ctx context.Context, trackID string, limit int) ([]domain.Track, error) {
	...
	err := r.q.QueryRowContext(ctx,
		`SELECT COALESCE(t.genre,''), al.artist_id
		 ...
		 WHERE t.id = ?`, trackID).Scan(&genre, &artistID)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
	...
	rows, err := r.q.QueryContext(ctx, `
		...
		ORDER BY (
			CASE
				WHEN al.artist_id = ?                                              THEN 8.0
				WHEN t.genre != '' AND t.genre = ?                                 THEN 5.0
				WHEN t.genre != '' AND ? != '' AND (
					t.genre LIKE '%'||?||'%' OR ?||'' LIKE '%'||t.genre||'%'
				)                                                                  THEN 3.0
				ELSE 1.0
			END * (0.5 + CAST(ABS(RANDOM()) % 1000 AS REAL) / 1000.0)
		) DESC
		LIMIT ?`,
		trackID, artistID, genre, genre, genre, genre, limit,
	)
	if err != nil {
		return nil, err
	}
